...
/**
 * Runs an inventory/item mutation for {@code target} on the correct thread: inline
 * when the command sender already <em>is</em> {@code target} (a normal command
 * executor already runs on that player's Folia region thread), otherwise hopped via
 * {@link Schedulers#entity} so entity/world API is touched safely.
 */
final class TargetActions {
