...
/**
 * SELIB fluent builder for menu {@link ItemStack}s with MiniMessage (hex-capable) display
 * names and lore. Italic decoration is disabled by default so names render as written.
 */
public final class ItemBuilder {
    ...
    /**
     * On a {@code PLAYER_HEAD}, textures the skull with {@code owner}'s skin. No-op for
     * non-skull items or a {@code null} owner.
     *
    ...
    /** Alias of {@link #skull(OfflinePlayer)}: textures a {@code PLAYER_HEAD} with {@code owner}'s skin. */
    public ItemBuilder head(OfflinePlayer owner) {
        return skull(owner);
    }

    /** Marks the item unbreakable (durability never decreases). */
    ...
    /**
     * Fully hides the item's hover tooltip. Uses {@link ItemMeta#setHideTooltip(boolean)}
     * (available on 1.21.9); falls back to hiding every {@link ItemFlag} on older APIs that
     * lack it.
     */
    ...
    /**
     * Sets the {@code custom_model_data} used by resource packs to swap the item's model.
     *
